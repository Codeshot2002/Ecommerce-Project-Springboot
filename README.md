...
A learning-focused, production-minded REST API for managing a product catalogue and placing inventory-safe orders. Built with Spring Boot, Spring Data JPA, and MySQL.

> The project is actively evolving. The focus so far is a clean backend foundation: layered design, validation, consistent errors, and safe stock updates.

## Highlights

- Product and category management APIs
- Order placement with request/response DTOs
- Atomic stock decrement that prevents overselling under concurrent checkout requests
- Transactional order creation: an order and its stock updates either all succeed or all roll back
- Optimistic locking on products and categories to prevent silent lost updates
- Bean Validation for request payloads
- Centralized REST exception handling

## Architecture

The application follows a classic layered architecture. Each feature area (for example Product, Category, Order, and future Cart/Auth/Address) follows the same request path.

```mermaid
flowchart LR
    B[Browser / API Client] --> C[REST Controllers]
    C --> S[Services]
...
| --- | --- |
| Controller | Defines HTTP endpoints, validates request DTOs, and returns HTTP responses. |
